...
简单解释一下以上命令的效果就是: 采用传统的 Make 方式编译 LLVM；编译 Release 版本（我们这里只是使用 llvm 工具链，不涉及开发，所以采用 Release 方式可以缩短编译时间和减少对硬盘的消耗，生成的可执行程序执行速度也快）；编译完成后如果要安装将安装在 llvm 源码树的 install 子目录下；只编译 RISCV 的 target；修改默认的 triple 组合为 riscv64-unknown-linux-gnu（避免在后面编译时再通过 --target 指定）；除了 llvm 外还会生成 clang、libcxx 和 libcxxabi。
...
的确是满足 64 位 RISC-V。然后将生成的 a.out 放到最小文件系统中的 /root 目录下，步骤简单总结如下：
...
QEMU 启动系统后，进入 `/root` 目录执行 `a.out`，能看到输出 "Hello, world!"。说明编译一切正常。
