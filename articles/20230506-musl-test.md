...
- `src/api`: 该目录包含了 “interface tests” 的测试用例，用于测试 musl 提供的头文件中的定义是否符合 POSIX 标准。musl 宣称支持 POSIX 2008（见 [musl wiki 之 Compatibility][6]）。
  ...
  仔细观察该目录下的 `.c` 文件，其内容的形式类似，都是先 `#include <xxx.h>`, 然后对这个头文件中的宏定义，结构体定义等进行检查。这些头文件就是 POSIX 2008 中定义的系统标准头文件（以 c 语言的形式），而检查的方法无非就是看 musl 定义的这些头文件是否符合规范的定义而已。这些检查自然需要先通过编译，最后会链接成一个 `main.exe`。
