...
LMUL 从字面意思来看是用于设置 cpu 如何对 vector register 进行分组，组装 RVV 中提出的一个新的概念，即 “虚拟寄存器”。
...
而且 LMUL 的取值还允许是分数（fractions），也即是说 "instead of grouping registers you can pick just a fraction of a register."
...
其中 LMUL*VLEN 可以认为是 RVV 引入的 “virtual” register 的宽度（单位是 bit），SEW 是我们 **期望的** 执行 vector 计算时每个 element 的宽度（单位是 bit），两者相除，得到的 VLMAX 就是我们 **期望的** 的一次 vector 计算可以处理的最大的 element 的个数。之所以强调是 **期望的**，因为这还不是最终 **实际** 运算时我们可以一次处理的 element 的个数。

## 5.5. VL

我们每次 vector 运算中 **实际** 可以处理的 element 的个数。
