...
本周期（2022/10/1 ~ 2022/10/13）RISCV 相关 merge PR 汇总参考 [这里][1]。
...
- https://android-review.googlesource.com/c/platform/bionic/+/2240296
- https://android-review.googlesource.com/c/platform/bionic/+/2240020
- https://android-review.googlesource.com/c/platform/bionic/+/2239866
...
- https://android-review.googlesource.com/c/platform/bionic/+/2240187
...
- 2240187: 对 versioner 这个工具程序增加了对 riscv 的支持。versioner 这个工具具体的作用我还没看清楚 TBD
...
- https://android-review.googlesource.com/c/platform/bionic/+/2243974
- https://android-review.googlesource.com/c/platform/bionic/+/2246733
- https://android-review.googlesource.com/c/platform/bionic/+/2247933
- https://android-review.googlesource.com/c/platform/bionic/+/2243973
- https://android-review.googlesource.com/c/platform/bionic/+/2242837
- https://android-review.googlesource.com/c/platform/bionic/+/2240019

详细说明：
- 2242573：`<sys/sat.h>`，为 `struct stat` 和 `struct stat64` 增加 riscv 的定义，由于目前 linux 上 32 位的 `struct stat` 和 64 位的 `struct stat64` 的定义是相同的，并没有针对 riscv32 的特殊 abi 定义，所以这里 Google 并没有采用 [RVI 仓库的方法][7]，只使用 `__riscv` 来区分，具体原因参考 [here][8]。
- 2243974：`<sys/user.h>` 增加 riscv 分支处理，目前为空，留着扩展
- 2246733：`<ucontext.h>` 增加 riscv 分支处理
- 2247933：`<sys/ucontext.h>` 增加 riscv 分支处理
- 2243973：`<fenv.h>`: 浮点数环境配置操作接口 fenv 的头文件 API 相关宏定义，ARCH 相关。
- 2242837：`<elf.h>`: ELF 头文件清理，趁着加入 riscv 的机会。
- 2240019: 针对 rename 和 renameat (`<stdio.h>`)，由于 riscv 不支持 renameat 这个系统调用，只支持 renameat2 这个系统调用，所以对所有 ARCH 统一采用 renameat2 来实现 rename/renameat 这些 libc 的 API。

## 2.4. libc 中的 TLS 支持

这些都是和 bionic 中支持 TLS（Thread Local Storage） 有关。具体涉及 PR 包括：

- https://android-review.googlesource.com/c/platform/bionic/+/2246833
- https://android-review.googlesource.com/c/platform/bionic/+/2243976

详细说明：  
- 2246833 是主要修改部分，涉及 libc 和 linker。
- 2243976 实现了 `__get_tls()` 的 riscv 版本，汇编

## 2.5. 其他未分类：

- https://android-review.googlesource.com/c/platform/bionic/+/2243977

  CFI unwind 相关， 支持 riscv

- https://android-review.googlesource.com/c/platform/bionic/+/2247578

  bionic 内部的私有头文件涉及 riscv 的分支增加

- https://android-review.googlesource.com/c/platform/bionic/+/2249439

  修改 BP 文件，实现构建 libdl for risc-v。

- https://android-review.googlesource.com/c/platform/bionic/+/2240297

  IFUNC 功能的调整和优化，趁着加入 riscv 的机会，有关 IFUNC 的介绍，可以参考笔记 [GNU IFUNC 介绍（RISC-V 版）][9]
...
- https://android-review.googlesource.com/c/kernel/tests/+/2252236

Add riscv64 for bpf test, fix vts_kernel_net_tests failed
...
- https://android-review.googlesource.com/c/toolchain/llvm_android/+/2238142

详细说明：

- 2237208/2239341/2237344:目前只看到改了一些简单的和构建相关的 python 脚本，增加了 riscv 这个 backend。
- 2238142: 升级到 r468909b 这个版本，支持 riscv target。
...
- https://android-review.googlesource.com/c/platform/system/core/+/2251616
- https://android-review.googlesource.com/c/platform/system/core/+/2251617
- https://android-review.googlesource.com/c/platform/system/core/+/2251619

详细说明：
- 2251616：Add riscv64 support for libcutils
- 2251617：Add code_coverage seccomp policy for riscv64
- 2251619：Add riscv64 support for debuggerd/crasher
...
- https://android-review.googlesource.com/c/platform/libcore/+/2252273
- https://android-review.googlesource.com/c/platform/frameworks/native/+/2252354
- https://android-review.googlesource.com/c/platform/frameworks/av/+/2252176
- https://android-review.googlesource.com/c/platform/system/libbase/+/2249256

# 7. 未归类的其他

- https://android-review.googlesource.com/c/platform/development/+/2251854
- https://android-review.googlesource.com/c/platform/system/memory/libmemunreachable/+/2249888
- https://android-review.googlesource.com/c/platform/external/strace/+/2162518
