...
- 2281221: 通过修改 `target/product/aosp_riscv64.mk`, 只编译那些支持 riscv64 的 modules，可以通过 `lunch aosp_riscv64-userdebug && m droid` 测试。
...
- 2281571：由于目前还没有 riscv64 的 apex 的 prebuilts，所以暂时用 arm64 的代替来确保测试通过，测试方法：`lunch aosp_riscv64-userdebug && m droid`
...
详细说明:
- 2246653: Add clang runtimes for riscv64, 测试方式：`lunch aosp_riscv64-userdebug && m ndk_sysroot bionic_sysroot_crt_objects`, 经尝试，这个是可以构建 pass 的 “#### build completed successfully (06:36 (mm:ss)) ####”。
